...
## GRAPH COLLECTION / DATA CHARACTERISTICS for OTHER VARIABLES: 

# 
mpls_precincts_shots %>% 
  ggplot(aes(x = perc_voted_no)) + geom_density()
...
# 
mpls_precincts_shots %>% 
  ggplot(aes(x = turnout)) + geom_density()

#
mpls_precincts_shots %>% 
  ggplot(aes(x = turnout, y = perc_voted_no)) + geom_point() + geom_smooth(method = "lm")

stargazer(lm(perc_voted_no ~ turnout, data = mpls_precincts_shots), type = "text")
...
stargazer(lm(turnout ~ log_n_shots_summer, data = mpls_precincts_shots %>% 
                    mutate(n_shots_summer = ifelse(n_shots_summer == 0, 1, n_shots_summer)) %>% 
                    mutate(log_n_shots_summer = log(n_shots_summer))), type = "text")

mpls_precincts_shots %>% 
  ggplot(aes(x = n_shots_summer, y = turnout)) + geom_point() + geom_smooth(method = "lm")

stargazer(lm(turnout ~ n_shots_summer, data = mpls_precincts_shots), type = "text")
...
#
mpls_precincts_shots %>% 
  ggplot(aes(x = vap_pop)) + geom_density()
...
#
mpls_precincts_shots %>% 
  ggplot(aes(x = turnout, y = perc_white)) + geom_point() + geom_smooth(method = "lm")
...
mpls_precincts_shots %>% 
  ggplot(aes(x = vap_perc_white)) + geom_density()
...
mpls_precincts_shots %>% 
  mutate(n_shots_summer = ifelse(n_shots_summer == 0, 1, n_shots_summer)) %>% 
  mutate(log_n_shots_summer = log(n_shots_summer)) %>% 
  ggplot(aes(x = log_n_shots_summer)) + geom_density()
...
mod <- lm(perc_voted_no ~ log_n_shots_summer + vap_perc_white, 
          data = mpls_precincts_shots %>% 
            rename("Log count of summer gunshots" = )
            mutate(n_shots_summer = ifelse(n_shots_summer == 0, 1, n_shots_summer)) %>% 
            mutate(log_n_shots_summer = log(n_shots_summer)))
